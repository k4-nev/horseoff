...
/** Тянет ли моя ступень это действие.
    ...
    Пока пороги не приехали — разрешаем. Это осознанный выбор, и он стоил
    одной ошибки: сначала было наоборот, «не знаем — запрещаем», и один
    неудавшийся запрос /api/roles молча убрал кнопку вложений у всех, включая
    владельца. Запрещает сервер, а здесь только подсказки; ошибиться в
    сторону лишней кнопки не страшно — на неё придёт понятный отказ с
    названием нужной ступени. Ошибиться в другую сторону — значит отобрать
    работающую функцию без единого слова. */

...
Здесь стережём пять вещей:
  ...
  5. недоделанное (dev.preview) доступно только владельцу и не открывается
     никакой правкой — правило общее на всё незаконченное в приложении.
...
check('порог редактора порогов не сдвинуть',
      ...
      str(refused))
taken, refused = R.set_thresholds({'dev.preview': 'common'})
check('недоделанное не открыть никому, кроме владельца',
      R.min_role('dev.preview') == 'arcana' and 'dev.preview' in refused,
      str(refused))
check('и это правило видно в интерфейсе как запертая строка',
      any(a['id'] == 'dev.preview' and a['locked'] for a in R.describe()['actions']))

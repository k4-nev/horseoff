...
// Builds straight into ../ (modules/valentine/) as valentine.js + valentine.css —
...
export default defineConfig({
  ...
  build: {
    ...
    emptyOutDir: false, // never wipe manifest.json / valentine_api.py sitting next to it
    cssCodeSplit: false,
    lib: {
      entry: resolve(__dirname, 'src/main.jsx'),
      formats: ['iife'],
      name: 'ValentineApp',
      fileName: () => 'valentine.js',
    },
    rollupOptions: {
      output: {
        // Force the single CSS asset to be named valentine.css instead of Vite's
        // default style.css / <libname>.css guess.
        assetFileNames: (info) => (info.name && info.name.endsWith('.css') ? 'valentine.css' : 'assets/[name][extname]'),
      },
    },
  },
});

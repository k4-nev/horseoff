const Channels = {
  ...
  // ─── Voice Rooms ───
  _voiceRooms: {},         // room_id -> {total, speaker_count, speakers:[]}
  _voiceRoomId: null,      // room we're currently in
  _voiceSpaceId: null,
  _voiceIsSpk: false,
  _voiceStream: null,      // local MediaStream
  _voicePeers: {},         // user_id -> RTCPeerConnection
  _voiceRemoteStreams: {}, // user_id -> MediaStream
  _voiceMuted: false,
  _voiceVideoMuted: true,
  _voiceHandRaised: false,
  _voicePanelMin: true,
  _voiceRoomData: null,    // full room snapshot {speakers, listeners}
  _voiceMyId: null,
  ...
  async joinVoiceRoom(spaceId, roomId, withVideo) {
    if (this._voiceRoomId && this._voiceRoomId !== roomId) {
      await this.leaveVoiceRoom();
    }
    if (this._voiceRoomId === roomId) {
      // Already in — just show panel
      this._voicePanelMin = false;
      this._renderVoicePanel();
      return;
    }
    // Request media
    try {
      var constraints = {audio: true, video: withVideo ? {width:{ideal:1280},height:{ideal:720}} : false};
      this._voiceStream = await navigator.mediaDevices.getUserMedia(constraints);
    } catch(e) {
    ...
    this._voiceVideoMuted = !withVideo;
  ...
  _voiceCleanup() {
    ...
    for (var uid in this._voicePeers) {
      try { this._voicePeers[uid].close(); } catch(e) {}
    }
    this._voicePeers = {};
    ...
    var panel = document.getElementById('chVoicePanel');
  ...
  _initPeer(userId, isInitiator) {
    var self = this;
    if (this._voicePeers[userId]) {
      try { this._voicePeers[userId].close(); } catch(e) {}
    }
    ...
    // Add local tracks
    if (this._voiceStream) {
      this._voiceStream.getTracks().forEach(function(t) {
        pc.addTrack(t, self._voiceStream);
      });
    }

    pc.ontrack = function(e) {
      if (!self._voiceRemoteStreams[userId]) {
        self._voiceRemoteStreams[userId] = new MediaStream();
      }
      self._voiceRemoteStreams[userId].addTrack(e.track);
      self._updateRemoteVideo(userId);
    };

    pc.onicecandidate = function(e) {
      if (e.candidate) {
        Shell.wsSend({type:'voice_ice', room_id: self._voiceRoomId, to_user_id: userId, candidate: e.candidate.toJSON()});
      }
    };
  ...
  _updateRemoteVideo(userId) {
    var vid = document.getElementById('chVoiceVid-' + userId);
    if (vid && this._voiceRemoteStreams[userId]) {
      vid.srcObject = this._voiceRemoteStreams[userId];
    }
  },

  _getVideoConstraints(speakerCount) {
    if (speakerCount <= 2) return {width:{ideal:1280},height:{ideal:720}};
    return {width:{ideal:640},height:{ideal:360}};
  },

  async _adaptVideoQuality() {
    if (!this._voiceStream || !this._voiceRoomData) return;
    var spkCount = (this._voiceRoomData.speakers || []).length;
    var constraints = this._getVideoConstraints(spkCount);
    var vtrack = this._voiceStream.getVideoTracks()[0];
    if (vtrack && !this._voiceVideoMuted) {
      try { await vtrack.applyConstraints(constraints); } catch(e) {}
    }
  },
  ...
  toggleVoiceMic() {
    ...
    if (this._voiceStream) {
      this._voiceStream.getAudioTracks().forEach(function(t){t.enabled = !this._voiceMuted;}.bind(this));
    }
  ...
  toggleVoiceVideo() {
    this._voiceVideoMuted = !this._voiceVideoMuted;
    if (this._voiceStream) {
      this._voiceStream.getVideoTracks().forEach(function(t){t.enabled = !this._voiceVideoMuted;}.bind(this));
    }
    Shell.wsSend({type:'voice_mute', room_id: this._voiceRoomId, muted: this._voiceMuted, video_muted: this._voiceVideoMuted});
    this._renderVoicePanel();
  },
  ...
  _renderVoicePanel() {
    ...
    var isSpk = this._voiceIsSpk;
    var isAdmin = this.isAdmin;

    // Get room name
    var roomName = '—';
    var channels = this.channelsBySpace[this._voiceSpaceId] || [];
    var ch = channels.find(function(c){return c.id===self._voiceRoomId});
    if (ch) roomName = ch.name;

    if (this._voicePanelMin) {
      // Minimized bar
      var spkHtml = spk.slice(0,4).map(function(p) {
        var av = p.avatar ? '<img src="data:image/png;base64,'+p.avatar+'" class="ch-vp-avatar">' : '<div class="ch-vp-avatar ch-vp-noav">'+self._esc(p.username[0]||'?')+'</div>';
        return av;
      }).join('');
      panel.className = 'ch-voice-panel ch-vp-min';
      panel.innerHTML =
        '<div class="ch-vp-info" onclick="Channels._voicePanelMin=false;Channels._renderVoicePanel()">' +
          '<span class="ico ico-14 ico-microphone" style="color:var(--accent)"></span>' +
          '<span class="ch-vp-roomname">'+this._esc(roomName)+'</span>' +
          '<div class="ch-vp-avatars">'+spkHtml+'</div>' +
          '<span class="ch-vp-count">'+spk.length+'/6</span>' +
        '</div>' +
        '<button class="ch-vp-btn ch-vp-leave" onclick="Channels.leaveVoiceRoom()" title="Выйти">✕</button>';
      return;
    }
    ...
    spk.forEach(function(p) {
      ...
      var av = p.avatar ? '<img src="data:image/png;base64,'+p.avatar+'" class="ch-vp-big-avatar">' : '<div class="ch-vp-big-avatar ch-vp-noav-big">'+self._esc(p.username[0]||'?')+'</div>';
      var muteIco = p.muted ? '<span class="ch-vp-mute-ico">🔇</span>' : '';
      videosHtml +=
        '<div class="ch-vp-tile" id="chVoiceTile-'+p.user_id+'">' +
          '<video id="chVoiceVid-'+p.user_id+'" autoplay playsinline '+(isMe?'muted':'')+' style="display:'+(p.video_muted?'none':'block')+'"></video>' +
          '<div class="ch-vp-tile-info" style="display:'+(p.video_muted?'flex':'none')+'">'+av+'</div>' +
          '<div class="ch-vp-tile-name">'+self._esc(p.username)+muteIco+'</div>' +
        '</div>';
    });
    ...
    var lstHtml = '';
    if (lst.length) {
      lstHtml = '<div class="ch-vp-listeners">';
      lst.forEach(function(p) {
        var av = p.avatar ? '<img src="data:image/png;base64,'+p.avatar+'" class="ch-vp-avatar">' : '<div class="ch-vp-avatar ch-vp-noav">'+self._esc(p.username[0]||'?')+'</div>';
        var handIco = p.raised_hand ? ' ✋' : '';
        var promoteBtn = isAdmin && p.raised_hand && spk.length < 6 ? '<button class="ch-vp-promote-btn" onclick="Channels.allowSpeak(\''+p.user_id+'\')">Дать слово</button>' : '';
        lstHtml += '<div class="ch-vp-listener">'+av+'<span class="ch-vp-lst-name">'+self._esc(p.username)+handIco+'</span>'+promoteBtn+'</div>';
      });
      lstHtml += '</div>';
    }

    // Controls
    var micIco = this._voiceMuted ? '🔇' : '🎤';
    var camIco = this._voiceVideoMuted ? '📷' : '📹';
    var micCls = this._voiceMuted ? ' ch-vp-btn-off' : '';
    var camCls = this._voiceVideoMuted ? ' ch-vp-btn-off' : '';
    var handCls = this._voiceHandRaised ? ' ch-vp-btn-on' : '';
    var handBtn = !isSpk ? '<button class="ch-vp-btn'+handCls+'" onclick="Channels.toggleVoiceHand()" title="Поднять руку">✋</button>' : '';

    panel.className = 'ch-voice-panel';
    panel.innerHTML =
      '<div class="ch-vp-header">' +
        '<button class="ch-vp-btn ch-vp-min-btn" onclick="Channels._voicePanelMin=true;Channels._renderVoicePanel()" title="Свернуть">—</button>' +
        '<span class="ch-vp-title">🔊 '+this._esc(roomName)+'</span>' +
        '<span class="ch-vp-stat">'+spk.length+'/6 говорят · '+(spk.length+lst.length)+' в комнате</span>' +
      '</div>' +
      '<div class="ch-vp-grid" id="chVoiceGrid">'+videosHtml+'</div>' +
      lstHtml +
      '<div class="ch-vp-controls">' +
        '<button class="ch-vp-btn'+micCls+'" onclick="Channels.toggleVoiceMic()" title="Микрофон">'+micIco+'</button>' +
        '<button class="ch-vp-btn'+camCls+'" onclick="Channels.toggleVoiceVideo()" title="Камера">'+camIco+'</button>' +
        handBtn +
        '<button class="ch-vp-btn ch-vp-leave" onclick="Channels.leaveVoiceRoom()" title="Покинуть">📴 Выйти</button>' +
      '</div>';

    // Attach local video
    setTimeout(function() {
      if (self._voiceStream) {
        var myVid = document.getElementById('chVoiceVid-' + myId);
        if (myVid) myVid.srcObject = self._voiceStream;
      }
      // Attach remote videos
      for (var uid in self._voiceRemoteStreams) {
        self._updateRemoteVideo(uid);
      }
    }, 50);
  },
  ...
  _onVoiceWS(data) {
    ...
    if (t === 'voice_state') {
      ...
      spk.forEach(function(p) {
        if (p.user_id !== self._voiceMyId) {
          self._initPeer(p.user_id, true);
        }
      });
    ...
    if (t === 'voice_joined') {
      ...
      if (!arr.find(function(p){return p.user_id===data.user_id})) {
        arr.push({user_id:data.user_id, username:data.username, avatar:data.avatar, muted:false, video_muted:true, raised_hand:false});
      }
      // If new speaker joined and we're a speaker too — initiate connection
      if (data.as_speaker && this._voiceIsSpk && data.user_id !== this._voiceMyId) {
        this._initPeer(data.user_id, false); // they will initiate
      }
      this._adaptVideoQuality();
      this._renderVoicePanel();
      return;
    ...
    if (t === 'voice_left') {
      ...
      if (this._voicePeers[data.user_id]) {
        try { this._voicePeers[data.user_id].close(); } catch(e) {}
        delete this._voicePeers[data.user_id];
        delete this._voiceRemoteStreams[data.user_id];
      }
      this._adaptVideoQuality();
      this._renderVoicePanel();
      return;
    ...
    if (t === 'voice_ice') {
      ...
      if (pc3 && data.candidate) {
        pc3.addIceCandidate(new RTCIceCandidate(data.candidate)).catch(function(e){});
      }
      return;
    }
    ...
    if (t === 'voice_mute_update') {
      ...
      var vid = document.getElementById('chVoiceVid-' + data.user_id);
      var tile = document.getElementById('chVoiceTile-' + data.user_id);
      if (vid && tile) {
        vid.style.display = data.video_muted ? 'none' : 'block';
        var info = tile.querySelector('.ch-vp-tile-info');
        if (info) info.style.display = data.video_muted ? 'flex' : 'none';
      }
      return;
    }
    ...
    if (t === 'voice_hand_raised') {
      if (this._voiceRoomData) {
        this._voiceRoomData.listeners.forEach(function(p) {
          if (p.user_id === data.user_id) p.raised_hand = data.raised;
        });
      }
    ...
    if (t === 'voice_promoted') {
      if (this._voiceRoomData) {
        ...
        if (found) {
          this._voiceRoomData.listeners = this._voiceRoomData.listeners.filter(function(p){return p.user_id!==data.user_id});
          found.muted = false; found.video_muted = true; found.raised_hand = false;
          this._voiceRoomData.speakers.push(found);
        }
      }
      if (data.user_id === this._voiceMyId) {
        this._voiceIsSpk = true;
        Shell.toast('Вам дали слово');
      }
      this._renderVoicePanel();
      return;
    }

...
// Собираем прямо в ../ (modules/mp/) под именами mp.js + mp.css — ровно те,
...
export default defineConfig({
  ...
  build: {
    ...
    emptyOutDir: false, // рядом лежат manifest.json и STATUS.md — не стирать
    cssCodeSplit: false,
    lib: {
    ...
    rollupOptions: {
      output: {
        assetFileNames: (info) => (info.name && info.name.endsWith('.css') ? 'mp.css' : 'assets/[name][extname]'),
      },
    },
  },
});

...
`Позиции 16:10`, `Позиции 20:00`. То есть несколько строк «Позиции» на разное
время + одна «Частотность».
...
следующий. В статус пишется, какой магазин сейчас проверяется.
...
заполнять **многоуровневый JSON-файл** со всеми проверками:
- позиция по каждому ключу каждого товара;
- кол-во товаров и кол-во ключей по каждому товару.
По этому JSON процесс понимает, на каком он этапе. Данные храним ~месяц и
отдаём в статистику. (Точная схема JSON и путь — уточняются, см. ниже.)
...
**Открытые вопросы (уточнить перед кодом):** редактирование расписания
(добавление/удаление строк, фикс. типы или свободные), связь частотности с
днём недели vs содержимым таблицы, точная схема и путь JSON-файла проверок,
семантика прогресса (по магазинам / внутри магазина), pause=StopTask vs
stop=InterruptTask, содержимое вкладки «Параметры» сейчас.

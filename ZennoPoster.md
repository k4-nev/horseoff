...
**Открытые вопросы (уточнить перед кодом):** как мост узнаёт ЖИВОЕ состояние
(текущий магазин, этап позиции/частотность, счётчик проверенных магазинов) —
через переменные ZP / файл состояния / инференс из JSON; точная схема JSON и
как в нём представлены «всего vs проверено» для прогресса; конвенция имени
файла по магазину (санитизация имени).
